...
pub fn find_cargo_projects(path: &Path, mut num_threads: usize, verbose: bool) -> Vec<ProjectDir> {
    if num_threads == 0 {
...
pub fn find_cargo_projects_task(job: Job, results: Sender<ProjectDir>, verbose: bool) {
    let path = job.0;
